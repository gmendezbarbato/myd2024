...
author: "FCS-UdelaR - Gustavo Méndez y Fabricio Carneiro"
...
.codefont[
```{r ej18a, eval = TRUE, echo = TRUE, warning=FALSE}
library(gapminder)
...
.codefontchico[
```{r ch9sdasss, echo = TRUE, message=FALSE, warning=FALSE}
library(rio)
...
.codefont[
```{r ch13___8, echo = TRUE, message=FALSE, warning=FALSE}
library(writexl)
...
```{r eaj13bb, echo = FALSE, results='hide', message=FALSE, warning=FALSE}
library(tidyverse)
```

.codefont[
```{r ej13bb, echo = TRUE, message=FALSE, warning=FALSE}
library(haven)
...
```{r ej13bca, echo = TRUE, message=FALSE, warning=FALSE}
library(haven)
data_uru <- read_dta("data_lapop/md_lapop_uruguay_2019.dta") %>% 
  select(fecha, q1, q2, pol1) %>% 
  as_factor()
